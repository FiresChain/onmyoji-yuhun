...
const gateLabels: Record<string,string> = { snapshotValid:"快照有效",targetsAndThresholdsValid:"单件决策已生成",staticPolicyConfirmed:"按当前决策生成",headerSourceValid:"Header 来源有效",groupLimitValid:"每码不超过 60 组",roundtripValid:"编码往返一致",previewComplete:"当前库存预演完成",retainedItemsProtected:"未误弃保留项" };
</script>
...
<template>
  ...
  <section class="header-source"><div><h2>Header 来源</h2><span>只从现有筛选码提取 16 字节 Header，不猜测。</span></div><textarea v-model="store.existingFilterCode" @input="store.invalidateHeader" rows="3" spellcheck="false" placeholder="粘贴任意一条当前账号可导入的筛选码"></textarea><button class="primary" :disabled="!store.analysis || !store.existingFilterCode || !!store.busy" @click="store.generatePlan"><Play :size="17" />生成并预演</button></section>
  ...
  <div v-else class="empty-state"><ShieldCheck :size="32" /><strong>尚未生成双码</strong><span>先完成账号分析，再提供当前账号的有效筛选码作为 Header 来源。</span></div>
</template>
