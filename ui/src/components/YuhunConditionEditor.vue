...
const props = defineProps<{ modelValue: FilterCriteria; readonly?: boolean }>();
...
<template>
  <section class="rule-field rule-yuhun-field"><div class="rule-yuhun-summary"><strong>御魂套装</strong><button v-if="!readonly" type="button" data-testid="open-rule-yuhun-picker" @click="pickerOpen = true"><Plus :size="15" />选择御魂</button></div><div class="rule-selected-yuhun"><span v-if="!modelValue.types.length">全部御魂套装</span><span v-for="name in modelValue.types" :key="name"><img v-if="yuhunImage(name)" :src="yuhunImage(name)!" :alt="yuhunDisplayName(name)" /><strong>{{ yuhunDisplayName(name) }}</strong></span></div></section>
  <RuleOptionGroup :readonly="readonly" label="位置" :options="numbers('号')" :selected="modelValue.positions" @toggle="toggle('positions', $event)" />
  <RuleOptionGroup :readonly="readonly" label="星级" :options="numbers('星')" :selected="modelValue.stars" @toggle="toggle('stars', $event)" />
  <RuleOptionGroup :readonly="readonly" label="等级" :options="levels" :selected="modelValue.levelRanges" @toggle="toggle('levelRanges', $event)" />
  <RuleOptionGroup :readonly="readonly" label="主属性" :options="stats" :selected="modelValue.mainStats" @toggle="toggle('mainStats', $event)" />
